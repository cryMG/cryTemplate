...
const examples = {
  ...
  wsDash: {
    tpl: [
      "Whitespace trimming with '-' (removes spaces + line breaks)",
      '',
      '1) No trim:',
      'Before',
      '  {{ name }}',
    ...
    data: {
      name: 'Alex',
      html: '<strong>Raw</strong> & <em>trimmed</em>',
    },
  },
  wsTilde: {
    tpl: [
      "Whitespace trimming with '~' (removes spaces, keeps line breaks)",
      '',
      'Before',
      '  {{~ name ~}}',
      'After',
      '',
      'Control tags with ~ (keeps the newline after %}):',
      '{%~ if show ~%}',
      '  Visible: {{~ name ~}}',
      '{%~ endif ~%}',
      'Done',
